...
public class ICamera {
    ...
    public static Camera.Size getNearestRatioSize(Parameters para,
        ...
        for (Camera.Size tmp : supportedSize) {
            if (tmp.height == 1280 && tmp.width == 720) {
                return tmp;
            }
            if (tmp.height == 960 && tmp.width == 540) {
                return tmp;
            }
        }
...
//					mCamera.getParameters(), screenWidth, screenHeight);
            Camera.Size bestPreviewSize = calBestPreviewSize(mCamera.getParameters(), screenWidth, screenHeight);
            ...
            params.setRotation(getCameraAngle(activity));//生成的图片转90°
...
//		int layout_height = (int) (layout_width * 1.0f * scale);
    ...
    public void startSurfacePreview(SurfaceHolder surfaceHolder) {
        try {
            if (mCamera != null) {
                try {
